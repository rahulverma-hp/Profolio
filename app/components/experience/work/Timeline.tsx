...
const DEFAULT_SUBTITLE_GAP = 0.6;
const BASE_YEAR_TO_TITLE = 0.58;

const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  const subtitleGap = point.subtitleGap ?? DEFAULT_SUBTITLE_GAP;
  const titleBlockY = -(BASE_YEAR_TO_TITLE + (point.yearTitleGap ?? 0));
    ...
    <group position={point.point} scale={isMobile ? 0.35 : 0.6}>
      ...
      <group>
        <group position={getPoint}>
          ...
          <group position={[0, titleBlockY, 0]}>
            <Text {...titleProps} fontSize={0.6} maxWidth={3.2} position={[0, -diff / 2, 0]}>
              {point.title}
            </Text>
            <Text
              {...textProps}
